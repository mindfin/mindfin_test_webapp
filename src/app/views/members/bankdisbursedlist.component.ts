...
@Component({
  selector: 'app-home',
  templateUrl: './bankdisbursedlist.component.html',
})
export class BankDisbursedListComponent {

  displayedColumns: string[] = ['date', 'cname', 'name', 'whosecase', 'executive', 'bank', 'amount', 'product', 'status', 'update'];
  samples: any;
  dataSource;
  ...
  getreport(obj) {
    ...
    this.commonservice.getcasedisburselist(this.postsPerPage, this.currentPage, this.sdate, this.edate);
    this.commonservice
  ...
  exportAsXLSX(): void {
    ...
    var a;
    const fileName = "Bank Disbursed list"; 
    for (let i = 0; i < come.length; i++) {
      this.array.push({

        "Created Date": this.samples[i].screateddate,
